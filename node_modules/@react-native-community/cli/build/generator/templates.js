...
var _copyProjectTemplateAndReplace = _interopRequireDefault(
  require('./copyProjectTemplateAndReplace')
);
...
function createProjectFromTemplate(
  ...
  const templatePath = _path().default.dirname(
    require.resolve('react-native/template')
  );

  (0, _copyProjectTemplateAndReplace.default)(
    templatePath,
    destPath,
    newProjectName
  );
...
function installTemplateDependencies(templatePath, destinationRoot) {
  ...
  const dependenciesJsonPath = _path().default.resolve(
    templatePath,
    'dependencies.json'
  );
  ...
  _logger.default.info(
    "Linking native dependencies into the project's build files..."
  );
...
function installTemplateDevDependencies(templatePath, destinationRoot) {
  ...
  // that are required by this template
  const devDependenciesJsonPath = _path().default.resolve(
    templatePath,
    'devDependencies.json'
  );

...
var _dependencies = _interopRequireDefault(
  ...
  return plugins.commands.reduce((acc, pathToCommands) => {
    ...
    const name =
      pathToCommands[0] === '@'
        ...
        : pathToCommands.split(_path().default.sep)[0];

    const pkg = require(_path().default.join(
      root,
      'node_modules',
      name,
      'package.json'
    ...
    const requiredCommands = require(_path().default.join(
      root,
      'node_modules',
      pathToCommands
    ));
    ...
    if (Array.isArray(requiredCommands)) {
      return acc.concat(
        requiredCommands.map(requiredCommand =>
          _objectSpread({}, requiredCommand, {
            pkg
          })
        )
      );

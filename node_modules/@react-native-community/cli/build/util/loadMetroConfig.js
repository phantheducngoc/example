...
var _findPlugins = _interopRequireDefault(require('../core/findPlugins'));

var _findSymlinkedModules = _interopRequireDefault(
  require('./findSymlinkedModules')
);
...
const resolveSymlinksForRoots = roots =>
  roots.reduce(
    (arr, rootPath) =>
      arr.concat((0, _findSymlinkedModules.default)(rootPath, roots)),
    [...roots]
  );
...
const getBlacklistRE = () =>
  (0, _metro().createBlacklist)([/.*\/__fixtures__\/.*/]);
/**
...
const getDefaultConfig = ctx => {
  ...
  return {
    resolver: {
      ...
      hasteImplModulePath: _path().default.join(
        ctx.reactNativePath,
        'jest/hasteImpl'
      )
    },
    serializer: {
      ...
      getPolyfills: () =>
        require(_path().default.join(ctx.reactNativePath, 'rn-get-polyfills'))()
    },
    server: {
    ...
    transformer: {
      ...
      assetRegistryPath: _path().default.join(
        ctx.reactNativePath,
 ...
 */
async function load(
  ctx, // $FlowFixMe - troubles with empty object being inexact
  options = {}
) {

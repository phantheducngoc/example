...
var _PackageManager = _interopRequireDefault(require('../util/PackageManager'));
...
const getLatestRNVersion = async () => {
  ...
  const { stdout } = await (0, _execa().default)('npm', [
    'info',
    'react-native',
    'version'
  ]);
...
const getPatch = async (currentVersion, newVersion, projectDir) => {
  ...
  const { name } = require(_path().default.join(projectDir, 'package.json'));

  _logger.default.info(
    `Fetching diff between v${currentVersion} and v${newVersion}...`
  );

  try {
    patch = await (0, _helpers.fetch)(
...
const getVersionToUpgradeTo = async (argv, currentVersion, projectDir) => {
  const newVersion = argv[0]
    ? _semver().default.valid(argv[0]) ||
      (_semver().default.coerce(argv[0])
        ? _semver().default.coerce(argv[0]).version
        : null)
...
const installDeps = async (newVersion, projectDir, patchSuccess) => {
  ...
  const pm = new _PackageManager.default({
    projectDir
  });
  const deps = [
...
async function upgrade(argv, ctx, args) {
  ...
  const { version: currentVersion } = require(_path().default.join(
    projectDir,
  ...
  try {
    ...
    await (0, _execa().default)('git', ['fetch', '--no-tags', tmpRemote]);
    ...
    await (0, _execa().default)('git', ['remote', 'remove', tmpRemote]);
